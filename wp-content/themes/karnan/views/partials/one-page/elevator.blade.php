@if (is_array($sections) && !empty($sections))
    ...
    @foreach ($sections as $elevatorKey => $elevatorItem)
        <li data-tooltip="{{ $elevatorItem['height_indicator'] }} - {{ $elevatorItem['section_title'] }}" data-tooltip-right class="elevator-item @if($elevatorKey == 0) is-first @endif @if($elevatorKey == count($sections)-1) is-last @endif ">
